...
//    [self.swipeRecognizer addTarget:self action:@selector(dismissingSwipeGestureRecognizerSwiped:)];
//    self.swipeRecognizer.delegate = self;
//    [self.scrollView addGestureRecognizer:self.swipeRecognizer];
}
...
/*
- (void)dismissingSwipeGestureRecognizerSwiped:(UISwipeGestureRecognizer *)swiper {
...
- (void)dismissImageWithFlick:(CGPoint)velocity {
    //NSLog(@"Flick:velocity = %f,%f",velocity.x,velocity.y);
    _flags.imageIsFlickingAwayForDismissal = YES;
    if (velocity.x<0)
    {
        _flickDiraction = ImageFlickDirectionLeft;
    }
    else
    {
        _flickDiraction = ImageFlickDirectionRight;
    }
    __weak JTSImageViewController *weakSelf = self;
